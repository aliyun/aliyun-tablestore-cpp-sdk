...
string OTSHelper::ISO8601TimeString()
{
    struct timeval tval;
    gettimeofday(&tval, NULL);

    char iso8601[28];
    time_t esec(tval.tv_sec);
    strftime(iso8601, sizeof(iso8601), "%Y-%m-%dT%H:%M:%S.", gmtime(&esec));
    snprintf(&iso8601[20], 8, "%06ldZ", tval.tv_usec);
...
string OTSHelper::Int64ToString(int64_t intValue)
{
    char buf[64];
...
string OTSHelper::BoolToString(bool value)
{
    if (value) {
        return "true";
    } else {
...
string OTSHelper::PrimaryKeyTypeToString(PrimaryKeyType type)
{
    switch (type) {
    case PKT_INTEGER:
        return "PKT_INTEGER";
    ...
    case PKT_BINARY:
        return "PKT_BINARY";
    default:
        return "UNKNOWN_TYPE(" + Int64ToString(type) + ")";
    }
}

string OTSHelper::ColumnTypeToString(ColumnType type)
{
    switch (type) {
    case CT_INTEGER:
        return "CT_INTEGER";
...
bool OTSHelper::StartsWith(const string& str, const string& prefix) {
